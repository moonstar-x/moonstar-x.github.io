...
export const NoArticlesAvailable: React.FC<Props> = ({ className, ...props }) => {
  return (
    <section className={clsx('border border-solid border-gray-400 rounded-[4px] p-[2rem] desktop:p-[4rem]', className)} {...props}>
      <h1 className="typography-h2 mb-[20px] text-center">
        {str('work.grid.no_posts.title')}
      </h1>

      <p className="text-justify desktop:text-center mb-[40px]">
        {str('work.grid.no_posts.description')}
      </p>

      <div className="text-center">
        <Button href={RouteDefs.home} color="white">
          {str('work.grid.no_posts.back_button')}
        </Button>
      </div>
    </section>
  );
